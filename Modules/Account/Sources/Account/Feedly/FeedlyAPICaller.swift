...
extension FeedlyAPICaller: FeedlyMarkArticlesService {
	...
	func mark(_ articleIds: Set<String>, as action: FeedlyMarkAction, completion: @escaping (Result<Void, Error>) -> ()) {
		...
		let chunkSize = 300
		var articleIdChunks: [[String]] = []
		
		// Split array into chunks
		for i in stride(from: 0, to: articleIdsArray.count, by: chunkSize) {
			let endIndex = min(i + chunkSize, articleIdsArray.count)
			let chunk = Array(articleIdsArray[i..<endIndex])
			articleIdChunks.append(chunk)
		}
