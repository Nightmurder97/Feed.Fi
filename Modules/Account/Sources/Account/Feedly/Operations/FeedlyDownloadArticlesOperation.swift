...
class FeedlyDownloadArticlesOperation: FeedlyOperation {
	...
	override func run() {
		...
		let articleIdsArray = Array(articleIds)
		
		// Split array into chunks
		for i in stride(from: 0, to: articleIdsArray.count, by: feedlyAPILimitBatchSize) {
			let endIndex = min(i + feedlyAPILimitBatchSize, articleIdsArray.count)
			let chunk = Array(articleIdsArray[i..<endIndex])
			
			let provider = FeedlyEntryIdentifierProvider(entryIds: Set(chunk))
